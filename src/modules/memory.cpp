  ...
  bool memory_module::update() {
    ...
    try {
      ...
      if (parsed.count("MemAvailable")) {
        ...
        kb_avail = kb_free + parsed["Buffers"] + parsed["Cached"] + parsed["SReclaimable"] - parsed["Shmem"];
      }
    ...
    m_perc_free_used = 100 - m_perc_memfree;
    ...
    if (m_label) {
      ...
      m_label->replace_token("%gb_free%", string_util::filesize_gb(kb_free, 2, m_bar.locale));
      m_label->replace_token("%gb_avail%", string_util::filesize_gb(kb_avail, 2, m_bar.locale));
      ...
      m_label->replace_token("%percentage_free_used%", to_string(m_perc_free_used));
      m_label->replace_token("%percentage_available%", to_string(m_perc_memavail));
      m_label->replace_token("%percentage_available_used%", to_string(m_perc_memused));
